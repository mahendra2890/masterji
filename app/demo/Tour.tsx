...
const SLIDES: Slide[] = [
  ...
  /* 3 ---------------------------------------------------------------- */
  {
          ...
          <div className={app.composer}>
            <Mark n={3} grow>
              <div className={`${app.composerInput} ${styles.ph}`}>
                Talk it through — nothing here counts until you file it under
                Today
              </div>
            </Mark>
            <span className={`${app.primaryBtn} ${styles.sendBtn}`}>Send</span>
          </div>
          {/* The one place the tour styles a borrowed screen itself: this
              line arrives with the two-boxes work, so on any base without it
              app.composerNote is undefined and the sentence renders as raw
              body text. A mock that only looks right on one branch is worse
              than three duplicated properties. */}
    ...
    notes: [
      ...
      <>
        The placeholder, and then the line under the box once you start typing.
        It&apos;s the one thing this screen has to keep saying.
      </>,
      <>
