...
Four secrets, created by hand because their values must not pass through a
...
```bash
# Render dashboard → masterji-api → Environment → reveal DJANGO_SECRET_KEY
printf '%s' '<render-DJANGO_SECRET_KEY>' | gcloud secrets create masterji-secret-key --data-file=- --project portfolio-502209
# The same Neon connection string Render already uses
printf '%s' '<neon-connection-string>'   | gcloud secrets create masterji-database-url --data-file=- --project portfolio-502209
printf '%s' '<google-oauth-client-secret>' | gcloud secrets create masterji-google-client-secret --data-file=- --project portfolio-502209
printf '%s' '<openai-api-key>'           | gcloud secrets create masterji-openai-api-key --data-file=- --project portfolio-502209
```
...
```bash
for s in masterji-secret-key masterji-database-url masterji-google-client-secret masterji-openai-api-key; do
...
CORS_ALLOWED_ORIGINS=https://masterji.mscsoftwares.in,\
...
gcloud run services update masterji-api --region asia-southeast1 --project portfolio-502209 \
  --update-env-vars "^@^DJANGO_ALLOWED_HOSTS=masterji.mscsoftwares.in,masterji-api-697438837887.asia-southeast1.run.app"
```
...
`/admin/` works directly with the existing admin user, since it is the same
database and — if §1 was followed — the same `SECRET_KEY`.
...
1. Point the frontend at Cloud Run — Vercel → masterji → Settings → Environment
   ...
   back.
2. Watch it. Both services are still running and the database is shared, so
   reverting is one environment variable.
3. Only then turn Render off. Suspending the service rather than deleting it
   keeps `render.yaml` meaningful and the rollback real for as long as you want
   it.

Until step 1, nothing a user touches has changed.
